...
class LabwareLoader:
    ...
    def get_tiprack(
        self,
        tiprack_type: Literal["biotix_96_tiprack_200ul", "biotix_96_tiprack_10ul"],
...
class OpenTronsProtocol:
    ...
    def exec(self) -> None:
        tiprack = self.labware_loader.get_tiprack("biotix_96_tiprack_200ul", "7")
        tiprack_2 = self.labware_loader.get_tiprack("biotix_96_tiprack_10ul", "8")
    ...
    def perform_pipetting_cycle(
        ...
        left_pipette.pick_up_tip(tiprack_2.wells_by_name()["A6"])
        self.messenger.send_message(
        ...
        # 90ulを全ウェルにロードする
        for n in range(1, 13):
            left_pipette.aspirate(5, pool.wells_by_name()["A1"])
            left_pipette.dispense(5, plate.wells_by_name()[f"A{n}"])
            left_pipette.blow_out()
        self.messenger.send_message(
            f"*{self.messenger.get_current_time()} ステータス→* DW150ulを全てのウェルにロードしました。"
        )

        left_pipette.drop_tip(tiprack_2.wells_by_name()["A6"])
        self.messenger.send_message(
            f"*{self.messenger.get_current_time()} ステータス→* 区画7のラックの6列目に300µLチップを戻しました。"
        )

        left_pipette.pick_up_tip(tiprack_2.wells_by_name()["A7"])
        self.messenger.send_message(
        ...
        left_pipette.aspirate(5, pool2.wells_by_name()["A1"])
        left_pipette.dispense(5, plate.wells_by_name()["A1"])
        left_pipette.mix(repetitions=3, volume=1.5)
        left_pipette.blow_out()

        for i in range(1, 12):
            left_pipette.aspirate(5, plate.wells_by_name()[f"A{i}"])
            left_pipette.dispense(5, plate.wells_by_name()[f"A{i+1}"])
            left_pipette.mix(repetitions=3, volume=1.5)
            left_pipette.blow_out()

        left_pipette.drop_tip(tiprack.wells_by_name()["A7"])
        self.messenger.send_message(
