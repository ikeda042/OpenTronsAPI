...
class OpenTronsProtocol:
    ...
    def exec(self) -> None:
        ...
        plate_1 = self.labware_loader.load_plate("corning_96_wellplate_360ul_flat", "6")

        for i in range(3):
            self.perform_pipetting_cycle(right_pipette, tiprack, plate_1)
    ...
    def perform_pipetting_cycle(
        ...
        self.messenger.send_message(
            f"*{self.messenger.get_current_time()}* 区画7のラックの1列目のチップを取りました。"
        )

        pipette.aspirate(200, plate.wells_by_name()["A1"])
        self.messenger.send_message(
            f"*{self.messenger.get_current_time()}* 1列目の全ウェルから200uLの溶液を吸引しました。"
        )

        pipette.dispense(200, plate.wells_by_name()["A2"])
        self.messenger.send_message(
            f"*{self.messenger.get_current_time()}* 2列目の全ウェルに200uLの溶液を移動しました。"
        )
